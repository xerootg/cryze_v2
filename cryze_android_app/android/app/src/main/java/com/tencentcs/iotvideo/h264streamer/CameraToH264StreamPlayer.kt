...
import com.tencentcs.iotvideo.h264streamer.H264StreamVideoRenderer
import com.tencentcs.iotvideo.h264streamer.IH264PlayerEventHandler
...
class CameraToH264StreamPlayer(override val cameraId: String, private val rtspPlaterEventHandler: IH264PlayerEventHandler, private val baseContext: MainActivity) : ICameraStream
{

    var h264Streamer: H264StreamVideoRenderer? = null
    ...
    private var watchdog = fixedRateTimer("watchdog", initialDelay = 30_000, period = 10_000) {
        ...
        if (watchdogEnabled && innerOnFrameCallback.frameCount == lastWatchdogFrameCount) {
            ...
            // kill the activity
            LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "watchdog timeout for ${cameraId}, killing activity")
    ...
    private fun refreshCameraCredentials(): Unit
    {
        ...
        LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "Getting camera credentials for camera id: $cameraId")
        ...
        val requestHandler = object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                {
                    LogUtils.e(CameraToH264StreamPlayer::class.simpleName, "Did not get a response body from the server")
                    return
                }

                // log the response body
                LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "Response body: $responseBody")

                if (!response.isSuccessful) {
                    LogUtils.e(CameraToH264StreamPlayer::class.simpleName, "Request failed with status code: ${response.code}")
    ...
    override fun start() {
        ...
        if (MessageMgr.getSdkStatus() != 1) {
            ...
            messageMgr.addAppLinkListener {
                LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "appLinkListener state = $it")
                var exitSetupLoop = false
                if (it == APP_LINK_ONLINE) {
                    LogUtils.i(
                        CameraToH264StreamPlayer::class.simpleName,
    ...
    private fun addSubscribeDevice(loginInfo: CameraCredential) {
        IoTVideoSdk.getNetConfig().subscribeDevice(loginInfo.accessToken, loginInfo.deviceId, object :
            ...
            override fun onSuccess(success: Boolean?) {
                ...
                h264Streamer = H264StreamVideoRenderer(loginInfo.socketPort, innerOnFrameCallback)

                iotVideoPlayer.setVideoDecoder(h264Streamer)
                ...
                iotVideoPlayer.setStatusListener { statusCode ->
                    ...
                    LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "Player connect mode: ${iotVideoPlayer.getConnectMode()}");
                    LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "Lan dev is ${if (isLanDevConnectable()) "" else "not "}connectable, streaming from internet")
                }
                ...
                LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "Player state: ${iotVideoPlayer.playState}");
            }
        })
    }
    ...
    override fun stop() {
        LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "Stopping player")
        try {
            iotVideoPlayer.stop()
            ...
            while (iotVideoPlayer.playState != PlayerStateEnum.STATE_STOP || iotVideoPlayer.playState == -1) {
                ...
                LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "Waiting for player to stop: ${iotVideoPlayer.playState} ($maxWaitTime ms remaining)")
                Thread.sleep(100)
            }

            iotVideoPlayer.release()
            h264Streamer?.release()
            h264Streamer = null
            unregisterSdk()
        } catch (e: Exception) {
            LogUtils.e(CameraToH264StreamPlayer::class.simpleName, "Error stopping player: ${e.message}")
        }
    }

    override fun release()
    {
        LogUtils.i(CameraToH264StreamPlayer::class.simpleName, "Releasing player")
        stop()
        watchdog.cancel()
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        other as CameraToH264StreamPlayer
    ...
    override fun toString(): String {
        ...
        stringBuilder.append("\n\tstreamerState=").append(h264Streamer?.toString())
        stringBuilder.append("\n}")
