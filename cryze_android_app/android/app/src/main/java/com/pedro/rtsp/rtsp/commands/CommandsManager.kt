...
import com.pedro.rtsp.rtsp.commands.SdpBody.createG711ABody
import com.pedro.rtsp.rtsp.commands.SdpBody.createDynamicAudioBody
    ...
    private fun createBody(): String {
        ...
        if (!audioDisabled) {
            audioBody = when (audioCodec) {
                ...
                // Exclusively 8khz, mono only
                AudioCodec.G711 -> createG711ABody(RtpConstants.trackAudio, sampleRate, isStereo)
                // These are special RTP "dynamic" payloads that handle any sample rate and channel count
                AudioCodec.PCMU -> createDynamicAudioBody(
                ...
                AudioCodec.PCMA -> createDynamicAudioBody(
                    RtpConstants.trackAudio,
                    sampleRate,
                    isStereo,
                    audioCodec.name
